...
class UserForm extends React.Component {
  ...
  componentWillMount() {
    ...
    if(userInfo){
      ...
      console.log('userInfo notok')
    }

    if(userInfo.status){
      console.log('status ok')
    }else{
      console.log('status notok')
    }
        ...
        <FormItem label="用户类别" {...formItemLayout}>
          {
            userInfo && type === 'detail' ? this.getUserType(userInfo.usertype) :
              getFieldDecorator('usertype', {
                initialValue: userInfo ? (userInfo.usertype ? userInfo.usertype : 0) : userInfo.usertype , 
                rules: [
                  {
                    required: true,
                    message: '请输入用户类别'
