...
import { Form, Input, Button, message,Icon,Modal,Checkbox,Select,Tabs } from "antd";
import Footer from "../../components/Footer";
import "./index.less";
import { connect } from "react-redux"; // 连接器
import { reqLogin,reqFindPwd,reqLoginSSO } from '../../api'
...
class Login extends React.Component {
  ...
  autologin = async(usercode,password) => {

    const result = await reqLoginSSO(usercode, password)
    // 登陆成功
    if (result.code===20000) {
      ...
      message.error(result.msg+",sso授权失败,跳转到系统登录首页")
      storageUtils.removeUser()
      memoryUtils.user = {}
  ...
  componentDidMount() {
    ...
    const param = this.props.location.search // '?username=vincentzhao&department=ITD&ssicid=001050908&staffid=01803'
    const arr = param.split('&') // ['?username=', 'f=7']
    const usercode = arr[0].substr(10) // '1'

    console.log(usercode);
    const failedCount = arr[1].substr(2) // '7'
    const result = this.autologin(usercode, 'ssologin');
    console.log(result);
    // if (result.code===20000) {
    //   // 将user信息保存到local
    //   const userresult = result.result.item_list
        ...
        <div className="login-content-wrap">
          <div className="login-content">
            ...
            <div className="login-box">
              ...
              <div className="title">工银澳门欢迎你</div>
              <LoginForm ref="login" />
            </div>
          </div>
        </div>
  ...
  handleSubmitFindPwd =  (e) => {
    ...
    this.userForm.props.form.validateFields(async (err,values)=>{
      if(!err){
        ...
        //   return;
        // }
        const result = await reqFindPwd(data.usercode,data.email,data.mobile);
        console.log(result);
        // const retdata = {}
        if (result.code === 20000) {
...
//添加修改密码表单
class FindPwdFormCopy extends React.Component {
  ...
  const prefixSelector = getFieldDecorator('prefix', {
    ...
    return (
      ...
      <Tabs defaultActiveKey="1" {...formItemLayout} onChange={this.handleChange}>
      <TabPane
        tab={
           <span>
             <Icon type="mail" />
...
FindPwdFormCopy = Form.create({})(FindPwdFormCopy);
