...
import ETableMutil from '../../../components/ETableMutil';
import BaseForm from '../../../components/BaseForm';
import {reqDailySummry} from '../../../api/index'
...
export default class DailyReport extends React.Component {
  ...
  formList = [
    ...
    // }, 
    {
      ...
      placeholder1: '请输入扣账开始日期',
      placeholder2: '请输入扣账截止日期',
    },
    {
      type: 'INPUT',
      label: '公司代码',
  ...
  requestList = async () => {
    ...
    let corpcode = this.params.corpcode;
    ...
    if((this.params.handleEndDate - this.params.handleBeginDate)/(3600000*24) > 31){
      Modal.info({
        title: "提示",
        content: "查询时间间隔不能大于31天"
      });
      return;
    }


    const retdata = await reqDailySummry(this.params.page,this.params.handleBeginDate.valueOf()
    ,this.params.handleEndDate.valueOf(),corpcode)
  ...
  handleOperate = async(type) => {
    ...
    if(type === 'exportExcel'){
      ...
      this.exportEXCELFile(selectedIds).then(
         (response) => {
          response.blob().then(blob => {
              let blobUrl = window.URL.createObjectURL(blob);
              let a = document.getElementById('a_id');
              let filename = moment(this.params.handleBeginDate).format(dateFormat) + '-daily.xls';
              a.href = blobUrl;
              a.download = filename;
              a.click();
  ...
  exportEXCELFile = (selectedIds) => fetch(
    '/report/dailysummry/exportexcel', {
        method: 'POST',
        body: JSON.stringify({ 
